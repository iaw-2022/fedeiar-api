...
const createGameWithCategories = async (request, response) => {
    ...
    try{
        ...
        if(errorCodes.duplicatedKey(error)){
            response.status(400).json({"message": "Game already exists. Please use another name", "code": 400});
        } else{
            response.status(500).json({"message": "Unknown server error.", "code": 500});
        }
        return;
...
const updateGame = async (request, response) => {
    ...
    try{
        ...
        if(errorCodes.invalidType(error)){
            ...
            response.status(400).json({"message": "Game already exists. Use another name", "code": 400}); 
        } else{
            response.status(500).json({"message": "Unknown server error.", "code": 500});
        }
    }
